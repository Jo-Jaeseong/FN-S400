...
### 공장 초기값에서 동국제약 RFID 이력 데이터 제외

- `userConfig` 초기값 테이블(`flash.c`)에 있던 S400(동국제약) 이관 데이터 중 RFID 카드 이력(카드값/잔량/등록연월 4슬롯)과 `BeforeRFID_DATA`(마지막 사용 슬롯 포인터)를 제거
  - 특정 고객사의 실사용 RFID 데이터가 다른 장비의 공장 초기값으로 그대로 배포되는 문제 방지 — 전 슬롯 빈 슬롯(0)으로 초기화
  - `Read_Flash()`의 기존 폴백 로직(`BeforeRFIDFlag==0`이면 1로 보정)으로 정상 동작 확인
